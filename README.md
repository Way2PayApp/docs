...
- [Детальное описание API эндпоинтов](#детальное-описание-api-эндпоинтов)
  ...
  - [8. Отмена заявки PayIn по ID](#8-отмена-заявки-payin-по-id)
  - [9. Получение списка заявок](#9-получение-списка-заявок)
  - [10. Получение заявки PayOut по ID](#10-получение-заявки-payout-по-id)
...
- [Обновления от 06.02.2026](#обновления-от-06022026)
- [Обновления от 21.04.2026](#обновления-от-21042026)
...
public static class Way2PaySignature
{
...
### 10. Получение заявки PayOut по ID
...
X-Webhook-Timestamp: 1713691200
X-Webhook-Signature: sha256=0123456789abcdef...
```
...
3. **Проверка подписи callback** - Проверять `X-Webhook-Signature` через `HMAC-SHA256` с использованием `callback_secret`
4. **Проверка timestamp** - Проверять `X-Webhook-Timestamp` (рекомендуемое окно 5 минут) для защиты от replay-атак
5. **Валидация данных** - Проверять корректность полученных данных
6. **Идемпотентность** - Обрабатывать возможные дублирующиеся уведомления
7. **Таймауты** - Отвечать на callback запросы в течение 30 секунд

#### Подпись callback
- Секрет подписи: `callback_secret` из настроек мерчанта
- Алгоритм: `HMAC-SHA256`
- Строка для подписи: `X-Webhook-Timestamp + "." + raw_json_body`
- Формат заголовка подписи: `X-Webhook-Signature: sha256=<hex>`

Пример проверки подписи на Node.js:
```javascript
const crypto = require('crypto');

function verifyWebhookSignature(rawBody, timestamp, signatureHeader, callbackSecret) {
...
```php
<?php
function verifyWebhookSignature(string $rawBody, string $timestamp, string $signatureHeader, string $callbackSecret): bool
{
    if (strpos($signatureHeader, 'sha256=') !== 0) {
        return false;
    }

    $expected = hash_hmac('sha256', $timestamp . '.' . $rawBody, $callbackSecret);
    $received = substr($signatureHeader, 7); // remove 'sha256='

    return hash_equals($expected, $received);
}
```

Пример проверки подписи на Python:
```python
import hmac
import hashlib

def verify_webhook_signature(raw_body: str, timestamp: str, signature_header: str, callback_secret: str) -> bool:
...
## Обновления от 06.02.2026
### Добавлены новые методы оплаты
- [Методы для PayIn](#методы-для-payin)

## Обновления от 21.04.2026
### Добавлена подпись callback-уведомлений для мерчантов
- Исходящие callback теперь подписываются через `HMAC-SHA256` с использованием `callback_secret` мерчанта.
- Добавлены заголовки `X-Webhook-Timestamp` и `X-Webhook-Signature`.
- Подписывается строка формата `timestamp.raw_json_body`.
- Рекомендуемая модель управления секретом: мерчант задает `callback_secret` в настройках (или админ заполняет от имени мерчанта).
